...
from integrated_viewer import build_hamner_sequence, muscle_paths_sequence, render_combined_mp4, render_layer_mp4, camera_preset
...
def _fit_skel_sequence(torch, model, motion, seed_fit, start_index=0, iterations=16, progress_cb=None, empirical_profile=None, acquisition_mode="monocular_depth_safe"):
    ...
    result={'version':'110.3.21.12-segment-gentle-anatomical-layers','retargeting_mode':'fullbody_segment_frame_reference_skel_skeleton_layer','scale':float(scale.cpu()),
...
def _skel_volumetric_anatomy_2010(joints, joint_names):
    """V110.3.21.12 · Generates volumetric anatomically-shaped bone and muscle meshes.
...
def _plot_skel_mesh_walker(mesh_seq):
    """V110.3.21.12 · visor de atlas anatómico REAL sobre SKEL. Sin fallback geométrico."""
    try:
        ...
        if not _ham_status.get("ready"):
            ...
            _orientation=st.selectbox(
                "Orientación del visor y del vídeo",
                ["Frontal","Posterior","Lateral derecha","Lateral izquierda","Oblicua clínica"],
                index=4,
                key="v32111_orientation"
            )
            _download_layer=st.selectbox(
                "Capa para descargar en vídeo",
                ["Piel","Esqueleto","Músculo estimado","Piel + Esqueleto","Esqueleto + Músculo","Todas"],
                index=5,
                key="v32111_download_layer"
            )
            _cam=camera_preset(_orientation)
            _js=_get_plotlyjs()

            _html=f"""<!doctype html><html><head><meta charset='utf-8'>
            <style>
            html,body{{margin:0;background:white;font-family:system-ui;color:#1f2937}}
            #bar{{display:flex;gap:6px;align-items:center;padding:6px;flex-wrap:wrap}}
            ...
            #slider{{flex:1;min-width:280px}}
            #plot{{height:730px;width:100%}}
            </style><script>{_js}</script></head><body>
            <div id='bar'>
              ...
              <button onclick='layer("all")'>Todas</button>
            ...
            <script>
            const P={_payload}; const gd=document.getElementById('plot'),sl=document.getElementById('slider'),co=document.getElementById('counter');
            let n=0,on=false,speed=1,last=0,raf=0;
            ...
            function mus(k){{let X=[],Y=[],Z=[];for(const q of P.M[k]){{for(const p of q){{X.push(p[0]);Y.push(p[1]);Z.push(p[2])}}X.push(null);Y.push(null);Z.push(null)}}return[X,Y,Z]}}
            let s=xyz(P.SV,0),b=xyz(P.BV,0),m=mus(0);
            const data=[
              {{type:'mesh3d',x:s[0],y:s[1],z:s[2],i:sfi,j:sfj,k:sfk,name:'Piel SKEL',opacity:.25,visible:true}},
              {{type:'mesh3d',x:b[0],y:b[1],z:b[2],i:bfi,j:bfj,k:bfk,name:'Esqueleto Hamner',opacity:1,visible:true}},
              {{type:'scatter3d',mode:'lines',x:m[0],y:m[1],z:m[2],name:'Músculo estimado',line:{{width:9}},visible:false}}
            ];
            Plotly.newPlot(gd,data,{{title:'Piel + OpenSim Hamner + Músculo',margin:{{l:0,r:0,t:45,b:0}},scene:{{aspectmode:'data',xaxis:{{range:P.ranges[0]}},yaxis:{{range:P.ranges[1]}},zaxis:{{range:P.ranges[2]}}}}}},{{responsive:true,displaylogo:false}});
            ...
            function frame(k){{n=(k+P.SV.length)%P.SV.length;s=xyz(P.SV,n);b=xyz(P.BV,n);m=mus(n);
            ...
            function loop(t){{if(!on)return;if(!last)last=t;if(t-last>1000/(25*speed)){{frame(n+1);last=t}}raf=requestAnimationFrame(loop)}}
            window.play=function(v){{speed=v;on=true;last=0;cancelAnimationFrame(raf);raf=requestAnimationFrame(loop)}};
            window.pause=function(){{on=false;cancelAnimationFrame(raf)}};
            </script></body></html>"""
            components.html(_html,height=805,scrolling=False)

            st.markdown("### 🎬 Vídeo · reproducción y descarga")
            st.caption(
                "El MP4 se genera desde los 75 frames ya calculados usando la orientación seleccionada arriba. "
                "Puedes descargar Piel, Esqueleto, Músculo estimado o cualquier combinación."
            )

            if st.button(
                f"🎬 Generar MP4 · {_download_layer} · {_orientation}",
                key="v32111_generate_layer_mp4",
                use_container_width=True
            ):
                try:
                    with st.spinner(f"Renderizando {_download_layer} en orientación {_orientation}…"):
                        _path=render_layer_mp4(
                            _download_layer,_orientation,
                            _Sv,_SF,_BV,_BF,_MSEQ,
                            fps=25
                        )
                        with open(_path,"rb") as _f:
                            st.session_state["v32111_layer_mp4"]=_f.read()
                        st.session_state["v32111_layer_mp4_name"]=(
                            f"PhysioSentinel_{_download_layer.replace(' ','_').replace('+','mas')}_"
                            f"{_orientation.replace(' ','_')}_75frames.mp4"
                        )
                except Exception as _e:
                    st.error(f"No se pudo generar el vídeo: {type(_e).__name__}: {_e}")

            if st.session_state.get("v32111_layer_mp4"):
                st.video(st.session_state["v32111_layer_mp4"])
                st.download_button(
                    "⬇️ Descargar MP4 seleccionado",
                    st.session_state["v32111_layer_mp4"],
                    st.session_state.get("v32111_layer_mp4_name","PhysioSentinel_video.mp4"),
                    "video/mp4",
                    key="v32111_download_layer_mp4",
                    use_container_width=True
                )

            st.warning(
                "La capa «Músculo estimado» representa trayectos musculares OpenSim/SKEL, no mallas musculares volumétricas reales. "
                "Hamner aporta geometría ósea de visualización biomecánica, no anatomía CT/MRI individual."
            )
...
def _load_mesh_sequence_npz_bytes_2012(raw):
    """V110.3.21.12 · Reabre un NPZ SKEL ya calculado sin volver a generar los 75 frames."""
    try:
...
def render_skel_poc_panel(motion, acquisition_mode="monocular_depth_safe", acquisition_label=None):
    ...
    st.caption("V110.3.21.12 · Carga el NPZ descargado de este registro para abrir directamente sus 75 frames SKEL. No repite fitting, retargeting ni generación de los 75 frames.")
